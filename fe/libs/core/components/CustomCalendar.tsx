...
interface CustomCalendarProps {
  ...
  onClose: () => void;
  /** Nếu truyền vào, các ngày từ maxDate trở đi sẽ bị disable (không chọn được). 
   *  Dùng `new Date()` để chặn ngày hôm nay và tương lai (chỉ cho phép quá khứ). */
  maxDate?: Date;
}

export const CustomCalendar = ({ value, onChange, open, anchorEl, onClose, maxDate }: CustomCalendarProps) => {
  ...
  /** Kiểm tra ngày có bị disable không (vượt quá maxDate) */
  ...
  for (let i = 1920; i <= maxYear; i++) years.push(i);
  ...
  return (
    ...
    >
          ...
          return (
            <Box
              ...
              sx={{
                ...
                '&:hover': {
                  backgroundColor: disabled ? 'transparent' : selected ? '#2f65f0' : '#f0f4ff',
                },
                ...(selected && !disabled && { color: '#fff', fontWeight: 600 }),
                ...(today && !selected && !disabled && { border: '1px solid #2f65f0', color: '#2f65f0' }),
      ...
      <Box sx={{ mt: 1.5, display: 'flex', justifyContent: 'space-between' }}>
        {/* Nếu có maxDate thì nút "Hôm nay" cũng bị disable */}
        <Button
          size="small"
          sx={{ textTransform: 'none', color: maxDate ? '#aaa' : '#2f65f0', fontSize: '0.75rem' }}
          onClick={() => { if (!maxDate) handleDateSelect(new Date()); }}
          disabled={!!maxDate}
        >
          Hôm nay
        </Button>
