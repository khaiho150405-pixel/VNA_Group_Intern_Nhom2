...
export const InjuryTypeView = React.forwardRef((props, ref) => {
  ...
  const handleSave = async () => {
    ...
    if (!form.name || form.name.trim() === "") {
      errors.name = "Vui lòng nhập tên loại chấn thương";
    }
    if (!form.level || isNaN(form.level)) {
      errors.level = "Vui lòng nhập cấp hợp lệ";
    }
    ...
    setLoading(true);
    try {
      const checkCodeRes: any = await injuryTypeService.checkCode(form.code, editId || undefined);
      if (checkCodeRes?.data === true || checkCodeRes === true) {
        setFormErrors({ code: "Mã số đã tồn tại" });
        setLoading(false);
        return;
      }
      ...
      if (editId) {
        await injuryTypeService.update(editId, form);
        enqueueSnackbar("Cập nhật thành công", { variant: "success" });
      } else {
        await injuryTypeService.create(form);
        ...
        <TableContainer sx={{ flex: 1, overflowY: 'auto' }}>
          ...
          <TableHead>
            <TableRow>
              ...
              <TableCell className={classes.headerCell} width={150}>Mã số</TableCell>
            ...
            <TableRow>
              ...
              <TableCell className={classes.filterCell}>
                <TextField
                  ...
                  size="small"
                  fullWidth
                  placeholder="Tên loại chấn thương"
                  value={filters.name}
              ...
              data.map((item) => (
                <TableRow key={item.id} hover selected={selectedIds.includes(String(item.id))}>
                  ...
                  <TableCell className={classes.bodyCell}>
                    {item.level > 1 ? " ".repeat((item.level - 1) * 4) + "--- " : ""}{item.name}
                  </TableCell>
                  <TableCell className={classes.bodyCell}>Cấp {item.level}</TableCell>
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers sx={{ p: 3, borderColor: "#eef0f4" }}>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
            ...
            <Box>
              <Typography sx={{ fontSize: "0.85rem", fontWeight: 500, color: "#94a3b8", mb: 0.5 }}>
                Tên loại chấn thương <span style={{ color: "red" }}>*</span>
              </Typography>
              <TextField
                fullWidth
                size="small"
