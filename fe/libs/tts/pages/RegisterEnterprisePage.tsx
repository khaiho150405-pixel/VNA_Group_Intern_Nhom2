...
export const RegisterEnterprisePage = () => {
  ...
  const handleEmailBlur = async () => {
    ...
    try {
      ...
      if (exists) setErrors((prev) => ({ ...prev, email: 'Email này đã được đăng ký' }));
    } catch (error) { }
  };
  ...
  const validateStep1 = (): boolean => {
    ...
    if (!formData.taxCode?.trim()) errs.taxCode = 'Mã số thuế không được để trống';
    else if (!isValidTaxCode(formData.taxCode)) errs.taxCode = 'Mã số thuế không hợp lệ';
    ...
    else if (!isValidEmail(formData.email)) errs.email = 'Email không đúng định dạng';
